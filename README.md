...
1. Run the light batch. In code cell 1, keep the recommended starting configuration above, then set `BATCH_PRESET = "light"`, `RUN_EXPERIMENTS = True`. Run code cells 1 to 8 in order. Code cell 6 only audits the current state, so missing canonical outputs before combine are expected.
